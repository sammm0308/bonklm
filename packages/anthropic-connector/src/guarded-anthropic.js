 ...
 */
import { GuardrailEngine, createLogger, Severity, createResult, createRetryPolicy, } from '@blackunicorn/bonklm';
import { VALIDATION_INTERVAL, DEFAULT_MAX_BUFFER_SIZE, DEFAULT_VALIDATION_TIMEOUT, } from './types.js';
/**
 ...
 * ```
 */
export function createGuardedAnthropic(client, options = {}) {
    ...
    onBlocked, onStreamBlocked, telemetry, circuitBreaker, enableRetry = true, maxRetries = 3, } = options;
     ...
     * @internal
     */
    const validateWithTimeout = async (content, context, runId) => {
     ...
     * @internal
     */
    const createValidatedStream = (stream, runId) => {
        if (validateStreaming && streamingMode === 'incremental') {
            // SEC-002: Incremental stream validation with early termination
            // SEC-003: Max buffer size enforcement
            return createIncrementalValidatedStream(stream, validateWithTimeout, maxStreamBufferSize, logger, onStreamBlocked, productionMode, telemetry, runId);
        }
 ...
 */
async function* createIncrementalValidatedStream(stream, validateWithTimeout, maxStreamBufferSize, logger, onStreamBlocked, productionMode, telemetry, runId) {
    let accumulatedText = '';
    ...
    try {
        for await (const event of stream) {
            ...
            if (event.type === 'content_block_delta' && event.delta?.type === 'text_delta') {
                ...
                if (accumulatedText.length + content.length > maxStreamBufferSize) {
                    ...
                    const error = new Error('Stream buffer exceeded maximum size');
        ...
        if (error instanceof Error && error.isStreamValidation !== true) {
            throw error;
        }
...
//# sourceMappingURL=guarded-anthropic.js.map