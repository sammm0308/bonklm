 ...
 * ```
 */
export function createGenkitGuardrailsPlugin(options = {}) {
    const { validators = [], guards = [], logger = DEFAULT_LOGGER, // DEV-002: Use proper logger
    validateFlowInput = true, validateFlowOutput = true, validateToolCalls = true, validateToolResponses = true, validateStreaming = false, streamingMode = 'incremental', // SEC-002: Default to incremental
    ...
    return {
         ...
         */
        beforeFlow: async (input, executionContext) => {
         ...
         */
        afterFlow: async (response, executionContext) => {
         ...
         */
        validateToolResponse: async (toolResponse, executionContext) => {
        ...
        _finalizeStream: async (accumulatedText, executionContext) => {
 ...
 * ```
 */
export function wrapFlow(flow, options = {}) {
    ...
    return async (input) => {
        ...
        const responseForValidation = response;
        ...
        if (!afterResult.allowed) {
            ...
            const fallback = typeof response === 'string'
                ...
                : { role: 'model', content: '[Content filtered by security policy]' };
            return fallback;
        }
        return response;
    };
}
//# sourceMappingURL=genkit-plugin.js.map