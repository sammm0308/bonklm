 ...
 * redactPIIValue('AB') // 'A*'
 */
export function redactPIIValue(value, patternName) {
    if (!value || value.length <= 1) {
        return '*'.repeat(value.length || 1);
    }
    // Pattern-specific redaction formats
    const patternRedactions = {
        SSN: () => '***-**-****',
        US_Phone: () => '(***) ***-****',
        Credit_Card: () => '**** **** **** ****',
        Email: (v) => {
 ...
 */
export async function redactPIIInString(content) {
    if (!content)
        return content;
 ...
 */
let cachedRedactionPatterns = null;
let redactionPatternsPromise = null;
async function getRedactionPatterns() {
    ...
    redactionPatternsPromise = (async () => {
        ...
        cachedRedactionPatterns = ALL_PATTERNS.map((p) => ({
 ...
 */
export function redactPIIInStringSync(content) {
    if (!content)
        return content;
    if (!cachedRedactionPatterns) {
 ...
 */
export function redactPIIInObject(obj) {
    ...
    for (const key of Object.keys(redacted)) {
        ...
        if (typeof value === 'string') {
            ...
            redacted[key] = value.map((item) => typeof item === 'string' ? redactPIIInStringSync(item) :
                typeof item === 'object' && item !== null ? redactPIIInObject(item) :
                    item);
        }
