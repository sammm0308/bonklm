 ...
 */
function getTestIdForCategory(category) {
    const testIds = {
        reflected: 'A03-201',
        stored: 'A03-202',
        domBased: 'A03-203',
 ...
 */
function isLineSafe(line, context) {
 ...
 */
export function detectXSS(content, context) {
    ...
    const hasCritical = detected.some((d) => ['script', 'javascript:', 'eval', 'data:text/html', 'onerror', 'onload']
        .some((p) => d.pattern.toLowerCase().includes(p.toLowerCase())));
    const severity = hasCritical ? Sev.CRITICAL : Sev.WARNING;
    return {
        hasXSS: true,
        severity,
        patterns: detected,
 ...
 */
export function checkXSS(content, context) {
 ...
 */
export function getXSSReport(content, context) {
    const guard = new XSSGuard({ context });
    return guard.getXSSReport(content);
}
//# sourceMappingURL=xss-safety.js.map