 ...
 */
function validateEnvPath(path) {
    // Check path length
    if (path.length > MAX_PATH_LENGTH) {
        throw new WizardError('PATH_TOO_LONG', `File path exceeds maximum length of ${MAX_PATH_LENGTH}`, 'Use a shorter file name');
    }
    // Disallow path traversal sequences (most critical security check)
    if (path.includes('..')) {
        throw new WizardError('INVALID_PATH', 'Invalid file path: path traversal detected', 'Path traversal sequences (..) are not allowed');
    }
    // Check for null bytes (another attack vector)
    if (path.includes('\0')) {
        throw new WizardError('INVALID_PATH', 'Invalid file path: null bytes detected', 'Null bytes are not allowed in file paths');
    }
    return path;
}
/**
...
export class EnvManager {
    ...
    async read() {
        ...
        try {
            ...
            throw new WizardError('ENV_READ_FAILED', `Failed to read .env file: ${this.path}`, 'Check file permissions and ensure the file is valid UTF-8 text', error, 1);
        }
    }
    /**
     ...
     */
    async write(entries, merge = true) {
        let content;
        if (merge) {
     ...
     */
    async ensureSameFilesystem(from, to) {
        try {
            ...
            if (stat1.dev !== stat2.dev) {
                throw new WizardError('CROSS_FILESYSTEM_RENAME', 'Cannot atomically rename across filesystems', 'Ensure .env is on the same filesystem as the temp directory', undefined, 1);
            }
        }
        catch (error) {
     ...
     */
    async setWindowsPermissions(filePath) {
        ...
        if (!normalizedPath.startsWith(resolve(cwd))) {
            throw new WizardError('PATH_OUTSIDE_DIRECTORY', 'File path is outside the allowed directory', 'File path must be within the project directory');
        }
        try {
            ...
            try {
                ...
                throw new WizardError('WINDOWS_PERMISSIONS_FAILED', 'Unable to set secure file permissions on Windows', 'Ensure icacls.exe is available or run with appropriate privileges', error, 1);
            }
        }
    }
    /**
    ...
    async verifyPermissions(filePath) {
        try {
            ...
            throw new WizardError('PERMISSION_VERIFICATION_FAILED', `File is not accessible: ${filePath}`, 'Check file permissions and ownership', error, 1);
        }
    }
    /**
    ...
    formatEntries(entries) {
        ...
        for (const [key, value] of Object.entries(entries)) {
            if (!validKeyPattern.test(key)) {
                throw new WizardError('INVALID_ENV_KEY', `Invalid environment variable key: ${key}`, 'Keys must start with letter/underscore and contain only alphanumeric/underscore characters', undefined, 1);
            }
            // Check for newlines in value (prevent multiline injection)
            if (value.includes('\n') || value.includes('\r')) {
                throw new WizardError('INVALID_ENV_VALUE', `Environment variable value contains newline: ${key}`, 'Remove newlines from environment variable values', undefined, 1);
            }
