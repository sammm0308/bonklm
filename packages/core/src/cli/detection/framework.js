 ...
 * ```
 */
export async function detectFrameworks(options = {}) {
    ...
    if (!normalizedRealPath.startsWith(normalizedWorkingDir)) {
        throw new WizardError('PATH_TRAVERSAL', 'package.json path resolved outside working directory', 'Ensure package.json is within the project directory', undefined, 1 // ERROR exit code
        );
    }
    ...
    try {
        ...
        if (fileStat.size > MAX_PACKAGE_JSON_SIZE) {
            throw new WizardError('FILE_TOO_LARGE', `package.json exceeds ${MAX_PACKAGE_JSON_SIZE} bytes`, 'Remove unused dependencies or split your package.json', undefined, 1 // ERROR exit code
            );
        }
    }
    catch (error) {
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(pkg, '__proto__') ||
            ...
            throw new WizardError('INVALID_PACKAGE_JSON', 'package.json contains prototype pollution', 'Remove malicious __proto__ or constructor properties', undefined, 1);
        }
 ...
 * ```
 */
export async function isFrameworkDetected(frameworkId, options) {
 ...
 * ```
 */
export async function getFrameworkVersion(frameworkId, options) {
    const frameworks = await detectFrameworks(options);
