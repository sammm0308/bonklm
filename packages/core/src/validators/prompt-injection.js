...
import { detectHiddenUnicode, normalizeText } from './text-normalizer.js';
 ...
 */
function detectMultiLayerEncoding(content, maxDepth = MAX_DECODE_DEPTH) {
    const findings = [];
 ...
 */
function iterativeDecode(input, processedInputs, maxDepth) {
    const decodeLayers = [];
 ...
 */
function detectHtmlCommentInjection(text) {
    const findings = [];
...
export class PromptInjectionValidator {
    ...
    logger;
    constructor(config = {}) {
        this.config = { ...DEFAULT_CONFIG, includeFindings: true, ...config };
 ...
 */
export function validatePromptInjection(content, config) {
 ...
 */
export function analyzePromptInjection(content, config) {
    const validator = new PromptInjectionValidator(config);
