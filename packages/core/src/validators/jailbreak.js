...
import { updateSessionState, } from '../session/SessionTracker.js';
import { getRegexCache } from './pattern-engine.js';
// =============================================================================
// CONSTANTS
// =============================================================================
/**
 ...
 */
export function fuzzyMatchKeywords(text, threshold = 0.75, cache) {
    const findings = [];
 ...
 */
function getHeuristicPatterns(cache) {
 ...
 */
export function detectHeuristicPatterns(text, cache) {
    const findings = [];
    // S016-001: Get or create regex cache
    const regexCache = cache ?? getRegexCache();
 ...
 */
export function detectJailbreakPatterns(content) {
    const findings = [];
 ...
 */
function getPatternNameCategory(name) {
    if (name.includes('dan'))
        return 'dan';
    if (name.includes('roleplay') || name.includes('character'))
        return 'roleplay';
    if (name.includes('hypothetical') || name.includes('educational'))
        return 'hypothetical';
    if (name.includes('developer') ||
        ...
        return 'authority';
    if (name === 'urgency_pressure' ||
        name === 'guilt_manipulation' ||
        name === 'flattery_attack' ||
        name === 'threat_pattern')
        return 'social_engineering';
    if (name === 'reciprocity_exploitation' ||
        name === 'social_proof_ai' ||
        name === 'consensus_pressure' ||
        name === 'politeness_exploitation' ||
        name === 'fitd_escalation')
        return 'social_compliance';
    if (name === 'false_rapport' ||
        name === 'boundary_erosion' ||
        name === 'shared_goal_framing' ||
        name === 'flattery_chain' ||
        name === 'guilt_induction' ||
    ...
    if (name === 'learned_helplessness' ||
        name === 'desperation_framing' ||
        name === 'moral_obligation' ||
        name === 'artificial_deadline')
        return 'emotional_manipulation';
    if (name.includes('grandma') ||
        name.includes('stan') ||
        name.includes('aim') ||
...
export class JailbreakValidator {
    ...
    constructor(config = {}) {
        ...
        this.config = { ...baseMerged, ...DEFAULT_JAILBREAK_CONFIG, ...config };
    ...
    analyze(content, sessionId) {
        ...
        const { riskScore, riskLevel, isEscalating } = this.calculateRisk(findings, fuzzyFindings, heuristicFindings, multiTurnFindings, sessionId);
        ...
        const { highestSeverity, shouldBlock } = this.calculateSeverityAndBlocking(findings, fuzzyFindings, heuristicFindings, multiTurnFindings, riskScore, isEscalating);
        return {
            findings,
     ...
     */
    mergeUniqueFindings(target, source) {
     ...
     */
    calculateRisk(findings, fuzzyFindings, heuristicFindings, multiTurnFindings, sessionId) {
     ...
     */
    calculateSessionRisk(findings, fuzzyFindings, heuristicFindings, multiTurnFindings, sessionId) {
     ...
     */
    buildSessionFindings(findings, fuzzyFindings, heuristicFindings, multiTurnFindings) {
     ...
     */
    calculateLocalRisk(findings, fuzzyFindings, heuristicFindings) {
     ...
     */
    determineRiskLevel(score, useSessionThresholds) {
     ...
     */
    applyEscalation(findings, isEscalating, riskScore) {
     ...
     */
    calculateSeverityAndBlocking(findings, fuzzyFindings, heuristicFindings, multiTurnFindings, riskScore, isEscalating) {
        const highestSeverity = this.getHighestSeverity(findings, fuzzyFindings, heuristicFindings, multiTurnFindings);
        const shouldBlock = highestSeverity === Severity.WARNING ||
     ...
     */
    getHighestSeverity(findings, fuzzyFindings, heuristicFindings, multiTurnFindings) {
        const severityOrder = {
            [Severity.INFO]: 0,
            [Severity.WARNING]: 1,
            [Severity.BLOCKED]: 2,
 ...
 */
export function validateJailbreak(content, config) {
 ...
 */
export function analyzeJailbreak(content, config, sessionId) {
    const validator = new JailbreakValidator(config);
