...
import { VALIDATION_INTERVAL, DEFAULT_MAX_BUFFER_SIZE, DEFAULT_VALIDATION_TIMEOUT, StreamValidationError, } from './types.js';
/**
 ...
 * ```
 */
export function createGuardedOllama(client, options = {}) {
     ...
     * @internal
     */
    const validateInput = async (prompt) => {
     ...
     * @internal
     */
    const createValidatedChatStream = (stream) => {
        if (validateStreaming && streamingMode === 'incremental') {
            // SEC-002: Incremental stream validation with early termination
            // SEC-003: Max buffer size enforcement
            return createIncrementalValidatedChatStream(stream, validateWithTimeout, maxStreamBufferSize, logger, onStreamBlocked, productionMode);
        }
     ...
     * @internal
     */
    const createValidatedGenerateStream = (stream) => {
        if (validateStreaming && streamingMode === 'incremental') {
            // SEC-002: Incremental stream validation with early termination
            // SEC-003: Max buffer size enforcement
            return createIncrementalValidatedGenerateStream(stream, validateWithTimeout, maxStreamBufferSize, logger, onStreamBlocked, productionMode);
        }
        // No streaming validation - return original stream
        return stream;
    };
    ...
    guardedClient.generate = async (opts) => {
        // Validate input first
 ...
 */
async function* createIncrementalValidatedChatStream(stream, validateWithTimeout, maxStreamBufferSize, logger, onStreamBlocked, productionMode) {
    let accumulatedText = '';
 ...
 * Implements SEC-002 (incremental validation) and SEC-003 (buffer size limit).
 */
async function* createIncrementalValidatedGenerateStream(stream, validateWithTimeout, maxStreamBufferSize, logger, onStreamBlocked, productionMode) {
    let accumulatedText = '';
    let validationCounter = 0;
    let model = '';
    let createdAt = new Date();
...
//# sourceMappingURL=guarded-ollama.js.map