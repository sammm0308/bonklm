...
export function createGuardedOpenAI(client, options = {}) {
     ...
     * @internal
     */
    const createValidatedStream = (stream) => {
        if (validateStreaming && streamingMode === 'incremental') {
            // SEC-002: Incremental stream validation with early termination
            // SEC-003: Max buffer size enforcement
            return createIncrementalValidatedStream(stream, validateWithTimeout, maxStreamBufferSize, logger, onStreamBlocked);
        }
 ...
 */
async function* createIncrementalValidatedStream(stream, validateWithTimeout, maxStreamBufferSize, logger, onStreamBlocked) {
    let accumulatedText = '';
...
//# sourceMappingURL=guarded-openai.js.map