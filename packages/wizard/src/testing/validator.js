...
export function validateTimeout(timeout) {
    if (timeout < 0) {
        throw new WizardError('INVALID_TIMEOUT', `Test timeout cannot be negative (received: ${timeout})`, 'Use a positive timeout value in milliseconds');
    }
    if (timeout > MAX_TEST_TIMEOUT) {
        throw new WizardError('TIMEOUT_TOO_LARGE', `Test timeout exceeds maximum (received: ${timeout}, maximum: ${MAX_TEST_TIMEOUT})`, `Use a timeout value less than ${MAX_TEST_TIMEOUT}ms`);
    }
    return timeout;
}
/**
 ...
 * ```
 */
export async function testConnector(connector, config, signal) {
 ...
 * ```
 */
export async function testConnectorWithTimeout(connector, config, timeout = DEFAULT_TEST_TIMEOUT) {
    ...
    try {
        ...
        if (isErrorName || isDOMException) {
            throw new WizardError('TEST_TIMEOUT', `Connector test timed out after ${validTimeout}ms`, 'Check your network connection or increase the timeout', isError ? error : undefined, ExitCode.PARTIAL);
        }
 ...
 * ```
 */
export async function testMultipleConnectors(tests, timeout = DEFAULT_TEST_TIMEOUT) {
    // Run all tests in parallel
    const results = await Promise.all(tests.map(async ({ connectorId, connector, config }) => {
 ...
 * ```
 */
export function validateConnectorConfig(connector, config) {
    const missing = [];
    const errors = [];
 ...
 */
export function createTestResult(connection, validation, error, latency) {
    return {
        connection,
        validation,
        error,
 ...
 */
export function isTestSuccessful(result) {
 ...
 */
export function isConnectionFailure(result) {
