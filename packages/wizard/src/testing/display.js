 ...
 * ```
 */
export function displayTestResults(tests, jsonMode = false) {
    if (jsonMode) {
 ...
 */
function getTestStatusSymbol(result, supportsColor) {
 ...
 * ```
 */
export function displaySingleTestResult(connectorId, connectorName, result, supportsColor) {
    ...
    console.log(`  ${useColor ? connColor : ''}Connection:${useColor ? COLORS.reset : ''} ${connSymbol} ${result.connection ? 'OK' : 'Failed'}`);
    ...
    if (result.connection) {
        ...
        console.log(`  ${useColor ? valColor : ''}Validation:${useColor ? COLORS.reset : ''} ${valSymbol} ${result.validation ? 'OK' : 'Failed'}`);
    }
    // Show latency
 ...
 * ```
 */
export function formatTestSummary(tests) {
 ...
 * ```
 */
export function displayTestSummary(summary) {
    const { supportsColor } = getTerminalCapabilities();
    console.log('');
    if (supportsColor) {
    ...
    console.log(`  ${supportsColor ? rateColor : ''}Success rate: ${summary.successRate}%${supportsColor ? COLORS.reset : ''}`);
}
/**
 ...
 * ```
 */
export function exportTestResultsJson(tests, pretty = true) {
 ...
 * ```
 */
export function exportTestSummaryJson(summary, pretty = true) {
 ...
 * ```
 */
export function formatTestDetail(test) {
 ...
 * ```
 */
export function getSuccessfulTests(tests) {
    return tests.filter((t) => t.result.connection && t.result.validation);
}
