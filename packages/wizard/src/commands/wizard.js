 ...
 * @returns Test result
 */
async function testSingleConnector(connectorId, config) {
    const connector = getConnector(connectorId);
    if (!connector) {
...
export const wizardCommand = new Command('wizard')
    ...
    try {
        ...
        if (p.isCancel(selected)) {
            throw new WizardError('USER_CANCELLED', 'Connector selection was cancelled', undefined, undefined, ExitCode.ERROR);
        }
        ...
        for (const connectorId of selected) {
            ...
            if (Object.keys(existingCredentials).length > 0) {
                ...
                if (p.isCancel(useExisting)) {
                    throw new WizardError('USER_CANCELLED', 'Credential selection was cancelled', undefined, undefined, ExitCode.ERROR);
                }
                if (useExisting) {
        ...
        if (successful.length > 0) {
            ...
            for (const r of successful) {
                p.log.log(`  - ${r.connectorName} (${r.result.latency}ms)`);
            }
        }
        if (failed.length > 0) {
            p.log.error(`Failed to configure ${failed.length} connector(s):`);
            for (const r of failed) {
                p.log.log(`  - ${r.connectorName}: ${r.result.error || 'Unknown error'}`);
            }
        }
        ...
        if (options.json) {
            console.log('\n' + JSON.stringify({
                ...
                envEntries: Object.fromEntries(Object.entries(envEntries).map(([k, v]) => [k, '***REDACTED***'])),
            }, null, 2));
        }
