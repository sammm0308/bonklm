 ...
 */
export function sanitizeError(error, depth = 0) {
    ...
    sanitizedMessage = sanitizedMessage.replace(/([A-Za-z0-9+/]{32,}={0,2})/g, (match) => isHighEntropy(match) ? '***REDACTED***' : match);
    // Pattern 2: Quoted high-entropy strings (more specific to avoid false positives)
    sanitizedMessage = sanitizedMessage.replace(/["']([a-zA-Z0-9_\-\.+/=]{32,})["']/g, (match, captured) => isHighEntropy(captured) ? '***REDACTED***' : match);
    // Pattern 3: Known JWT format (header.payload.signature)
    sanitizedMessage = sanitizedMessage.replace(/eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/g, '***JWT_REDACTED***');
    ...
    if (sanitizedStack) {
        ...
        // Base64 patterns
        sanitizedStack = sanitizedStack.replace(/([A-Za-z0-9+/]{32,}={0,2})/g, (match) => isHighEntropy(match) ? '***REDACTED***' : match);
    }
...
export class WizardError extends Error {
     ...
     */
    constructor(code, message, suggestion, cause, exitCode) {
        super(message);
        this.code = code;
