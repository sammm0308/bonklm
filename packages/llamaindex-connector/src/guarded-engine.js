...
exports.createGuardedQueryEngine = createGuardedQueryEngine;
exports.createGuardedRetriever = createGuardedRetriever;
 ...
 * ```
 */
function createGuardedQueryEngine(queryEngine, options = {}) {
    const { validators = [], guards = [], logger = DEFAULT_LOGGER, validateRetrievedDocs = true, onBlockedDocument = 'filter', productionMode = process.env.NODE_ENV === 'production', validationTimeout = types_js_1.DEFAULT_VALIDATION_TIMEOUT, maxRetrievedDocs = types_js_1.DEFAULT_MAX_RETRIEVED_DOCS, onQueryBlocked, onDocumentBlocked, onResponseBlocked, } = options;
     ...
     * @internal
     */
    const validateDocuments = async (nodes) => {
    ...
    return {
        ...
        async query(queryStr, options = {}) {
            ...
            if (!responseResult.allowed) {
                ...
                if (onResponseBlocked)
                    onResponseBlocked(responseResult);
                return {
 ...
 * ```
 */
function createGuardedRetriever(retriever, options = {}) {
    const { validators = [], guards = [], logger = DEFAULT_LOGGER, validateRetrievedDocs = true, onBlockedDocument = 'filter', productionMode = process.env.NODE_ENV === 'production', validationTimeout = types_js_1.DEFAULT_VALIDATION_TIMEOUT, maxRetrievedDocs = types_js_1.DEFAULT_MAX_RETRIEVED_DOCS, onQueryBlocked, onDocumentBlocked, } = options;
    const engine = new bonklm_1.GuardrailEngine({
        validators,
        guards,
        logger,
    ...
    return {
         ...
         */
        async retrieve(queryStr, options = {}) {
            // Validate the query
...
//# sourceMappingURL=guarded-engine.js.map