...
function displayResults(results, config) {
  ...
  for (const [key, framework] of Object.entries(results.frameworks)) {
    ...
    const color = framework.coverage >= 90 ? '\x1b[32m' : framework.coverage >= config.minimum ? '\x1b[33m' : '\x1b[31m';
    const reset = '\x1b[0m';

    console.log(`  ${status} ${framework.short.padEnd(6)} ${bar} ${color}${framework.coverage.toFixed(1).padStart(5)}%${reset} (${framework.implemented}/${framework.total})`);

    if (config.verbose && framework.testIds.length > 0) {
      console.log(`       Tests: ${framework.testIds.slice(0, 10).join(', ')}${framework.testIds.length > 10 ? '...' : ''}`);
    }
  }
  ...
  const status = results.summary.overallCoverage >= 90 ? '✓' : results.summary.overallCoverage >= config.minimum ? '~' : '✗';
  const color = results.summary.overallCoverage >= 90 ? '\x1b[32m' : results.summary.overallCoverage >= config.minimum ? '\x1b[33m' : '\x1b[31m';
  const reset = '\x1b[0m';

  console.log('  ──────────────────────────────────────────────────────────────────────');
  console.log(`  ${status} OVERALL ${summaryBar} ${color}${results.summary.overallCoverage.toFixed(1).padStart(5)}%${reset} (${results.summary.totalImplemented}/${results.summary.totalControls})`);
  console.log('═'.repeat(70));
  console.log('');
...
function main() {
  ...
  try {
    ...
    if (!config.json && !passed) {
      console.error(`\x1b[31m[FAILURE]\x1b[0m Coverage ${results.summary.overallCoverage.toFixed(1)}% is below minimum ${config.minimum}%`);
