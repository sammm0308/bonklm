...
function checkBundleSizes(config) {
  ...
  for (const bundle of bundleDirs) {
    ...
    if (sizeMB > config.threshold) {
      ...
      results.warnings.push(`${bundle.name} approaching threshold: ${formatBytes(totalSize)} (${percentOfThreshold.toFixed(1)}%)`);
    }
  ...
  console.log('Summary');
  console.log('='.repeat(60));
  console.log(`Total bundle size: ${formatBytes(results.totalSize)} (${(results.totalSize / (1024 * 1024)).toFixed(2)} MB)`);
