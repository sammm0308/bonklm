...
export async function installCommand(options) {
  ...
  const cleanupHandler = async () => {
    console.log('\n');
    spinner.stop('Installation interrupted');
    await rollback();
    process.exit(130);
  };
  ...
  try {
    ...
    spinner.stop('Step 1/6: Download complete');
    ...
    if (!options.fromGit) {
      ...
      if (extractResult.cancelled) {
        spinner.stop('Installation cancelled by user');
        process.exit(0);
      }

      installState.extractedFiles = extractResult.files || [];
      spinner.stop(`Step 2/6: Extracted ${extractResult.filesExtracted} files`);
    } else {
      spinner.stop('Step 2/6: Files copied from repository');
    }
    ...
    spinner.stop('Step 3/6: Package.json configured');
    ...
    if (!options.skipNpmInstall && !options.dryRun) {
      ...
      try {
        ...
        spinner.stop('Step 4/6: Dependencies installed');
        ...
        spinner.stop('Step 4/6: npm install had issues (you may need to run manually)');
    ...
    if (!options.skipWizard && !options.yes && !options.dryRun) {
      spinner.stop('Step 5/6: Starting setup wizard...');
      console.log('');
    ...
    if (!options.dryRun) {
      ...
      if (healthResult.success) {
        spinner.stop('Step 6/6: Installation verified');
      } else {
        spinner.stop('Step 6/6: Health check had warnings');
        healthResult.warnings.forEach(w => logger.warn(`  - ${w}`));
      }
    } else {
      spinner.stop('Step 6/6: Skipped health check (dry run)');
    }
    ...
    spinner.stop(`Installation failed: ${error.message}`);
...
function showQuickStart(isDryRun) {
  if (isDryRun) {
    ...
    console.log(pc.bold(pc.cyan('  DRY RUN COMPLETE')));
    console.log(pc.bold(pc.cyan('='.repeat(60))));
    console.log('');
  ...
  console.log(pc.bold(pc.green('  BMAD-CYBER INSTALLED SUCCESSFULLY')));
  console.log(pc.bold(pc.green('='.repeat(60))));
  console.log('');
  ...
  console.log(pc.cyan('     /agents/abdul'));
  console.log('');
