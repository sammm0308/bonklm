 ...
 */
import { detectPatterns, CRITICAL_PATTERNS, } from './pattern-engine.js';
// =============================================================================
 ...
 */
export function detectCodeFormatInjection(content) {
    ...
    for (const { text, source } of commentExtracts) {
        ...
        for (const pf of patternFindings) {
            ...
            const severity = isCriticalPattern ? 'WARNING' : 'INFO';
            findings.push({
                category: 'code_format_injection',
 ...
 */
export function detectCharacterLevelEncoding(content) {
 ...
 */
export function detectContextOverload(content) {
    if (!content || content.trim().length === 0) {
        return [];
    }
    const findings = [];
 ...
 */
export function detectMathLogicEncoding(content) {
