 ...
 */
export function isSensitiveContext(content, line) {
 ...
 */
export function isFakeData(content, line) {
    // Check the line itself
 ...
 */
export function validatePiiGuard(content, filePath) {
    if (!content) {
    ...
    if (overrideResult.valid) {
        ...
        printOverrideConsumed(`${detections.length} PII pattern(s) detected`, 'BMAD_ALLOW_PII');
        return EXIT_CODES.ALLOW;
    }
    // Block the operation
