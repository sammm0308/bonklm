 ...
 */
export function extractPathsFromCommand(cmd) {
    const paths = [];
    const seen = new Set();
 ...
 */
export function detectUnsafeSubstitutions(cmd) {
    const unsafe = [];
 ...
 */
export function checkBashCommand(cmd, cwd) {
 ...
 */
export function checkFilePath(filePath, cwd) {
    const projectDir = getProjectDir();
    if (!filePath) {
 ...
 */
export function validateOutsideRepo(input) {
...
//# sourceMappingURL=outside-repo.js.map