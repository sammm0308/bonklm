 ...
 */
export function validateRequiredField(input, trimWhitespace = true) {
 ...
 * @returns ValidationResult indicating success or failure
 */
export function validateStringLength(str, min = 0, max) {
    // Type validation
 ...
 * @returns ValidationResult indicating success or failure
 */
export function validateFileType(filename, allowedTypes = DEFAULT_CONFIG.allowedMimeTypes) {
...
function validateValueAgainstSchema(value, schema) {
    ...
    if (schema.type === 'object' && schema.properties && typeof value === 'object' && value !== null) {
        const nestedResult = validateJsonSchema(value, schema);
 ...
 * @returns ValidationResult indicating success or failure
 */
export function validateUnicodeNormalization(str, targetForm = 'NFC') {
    if (typeof str !== 'string') {
        return {
            valid: false,
            error: `Expected string but received ${typeof str}`,
 ...
 */
function detectUnicodeHomographs(str) {
    const detected = [];
 ...
 * @returns ValidationResult indicating success or failure
 */
export function validateSqlInjection(input, context) {
 ...
 * @returns ValidationResult indicating success or failure
 */
export function validateXss(input, context) {
    if (context === 'code' || context === 'documentation') {
 ...
 * @returns ValidationResult indicating success or failure
 */
export function validateCommandInjection(input, context) {
    if (context === 'code' || context === 'documentation') {
        return { valid: true, error: null };
    }
...
export function validateInput(input, rules) {
    ...
    if (typeof input === 'string') {
        ...
        if (rules.minLength !== undefined || rules.maxLength !== undefined) {
            const lengthResult = validateStringLength(input, rules.minLength ?? 0, rules.maxLength ?? Number.MAX_SAFE_INTEGER);
    ...
    if (typeof input === 'number') {
        ...
        if (rules.min !== undefined || rules.max !== undefined) {
            const rangeResult = validateNumericRange(input, rules.min ?? Number.MIN_SAFE_INTEGER, rules.max ?? Number.MAX_SAFE_INTEGER);
...
//# sourceMappingURL=asvs-input-validator.js.map