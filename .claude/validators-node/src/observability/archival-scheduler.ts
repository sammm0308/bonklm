...
export class ArchivalScheduler {
  private configManager: ArchivalConfigManager;
  private statusFile: string;
  private logFile: string;
  private isInitialized = false;
  private currentJob: Promise<ArchivalJobResult> | null = null;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      await AuditLogger.log(
        'archival_scheduler',
        'INITIALIZED',
        { statusFile: this.statusFile, logFile: this.logFile },
        'INFO'
      );
   ...
   */
  async runArchivalJob(): Promise<ArchivalJobResult> {
   ...
   */
  private async executeArchivalJob(): Promise<ArchivalJobResult> {
   ...
   */
  async getStatus(): Promise<ArchivalJobStatus> {
    try {
   ...
   */
  private async saveStatus(status: ArchivalJobStatus): Promise<void> {
   ...
   */
  private async logJobResult(result: ArchivalJobResult): Promise<void> {
    try {
      const logEntry = {
   ...
   */
  private async sendFailureAlert(status: ArchivalJobStatus, result: ArchivalJobResult): Promise<void> {
    await AuditLogger.log(
      'archival_scheduler',
      'ALERT_FAILURE',
      {
   ...
   */
  private calculateNextArchivalTime(): string {
   ...
   */
  async getJobHistory(limit: number = 10): Promise<ArchivalJobResult[]> {
    try {
      ...
      const jobs = lines
        ...
        .filter((entry): entry is ArchivalJobResult => entry !== null)
   ...
   */
  async cleanupJobHistory(retainDays: number = 30): Promise<void> {
    try {
   ...
   */
  async cancelRunningJob(): Promise<boolean> {
    const status = await this.getStatus();
 ...
 */
let globalScheduler: ArchivalScheduler | null = null;
 ...
 */
export async function initializeArchivalScheduling(): Promise<void> {
  try {
    ...
    throw new ArchivalError(
      `Failed to initialize archival scheduling: ${error instanceof Error ? error.message : String(error)}`,
 ...
 */
export async function triggerArchival(): Promise<ArchivalJobResult> {
  const scheduler = getArchivalScheduler();
