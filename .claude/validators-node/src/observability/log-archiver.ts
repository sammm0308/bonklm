...
export class S3ArchivalError extends ArchivalError {
  constructor(message: string, public readonly awsError?: Error) {
    super(message, 'S3_ARCHIVAL_ERROR');
  }
}

export class GPGSigningError extends ArchivalError {
  constructor(message: string, public readonly gpgError?: Error) {
...
export class LogArchiver {
   ...
   */
  private async verifyObjectLockConfiguration(): Promise<void> {
    try {
      ...
      if (error.name === 'ObjectLockConfigurationNotFoundError' ||
        ...
        throw new ArchivalError(
          `S3 bucket ${this.config.bucket} does not have Object Lock enabled. This is required for compliance.`,
          'OBJECT_LOCK_NOT_ENABLED'
        );
      }
      throw new S3ArchivalError(
        `Failed to verify Object Lock configuration: ${error.message || String(error)}`,
        error
      );
    }
  }
  ...
  static fromEnvironment(): LogArchiver {
    ...
    if (!bucket) {
      throw new ArchivalError(
        'BMAD_S3_ARCHIVE_BUCKET environment variable is required',
        'MISSING_CONFIG'
      );
    }
    ...
    const config: ArchivalConfig = {
      ...
      encryptionType: (process.env.BMAD_S3_ENCRYPTION_TYPE as any) || 'SSE-S3',
   ...
   */
  private generateArchiveId(startDate: Date, endDate: Date): string {
   ...
   */
  private async getLogFilesInRange(startDate: Date, endDate: Date): Promise<Array<{path: string, size: number}>> {
    ...
    try {
      ...
      throw new ArchivalError(
        `Failed to scan log directory: ${error instanceof Error ? error.message : String(error)}`,
        'LOG_SCAN_ERROR'
      );
    }
   ...
   */
  private async createCompressedArchive(logFiles: Array<{path: string, size: number}>): Promise<Buffer> {
      ...
      const batchResults = await Promise.all(
        batch.map(async (logFile) => {
          try {
            ...
            const processedEntries: AuditLogEntry[] = [];
            ...
            for (let lineIndex = 0; lineIndex < lines.length; lineIndex += LINE_BATCH_SIZE) {
              ...
              processedEntries.push(...batchEntries.filter((entry): entry is AuditLogEntry => entry !== null));
            }
            ...
            throw new ArchivalError(
              `Failed to process log file ${logFile.path}: ${error instanceof Error ? error.message : String(error)}`,
   ...
   */
  private async getLastArchiveHash(): Promise<string | undefined> {
    try {
   ...
   */
  private async signWithGPG(data: Buffer, _metadata: ArchiveMetadata): Promise<string> {
    if (!this.config.gpgSigningKey) {
    ...
    return new Promise((resolve, reject) => {
      ...
      gpg.on('close', (code: number | null) => {
        if (code === 0 && signature.trim()) {
          ...
          reject(new GPGSigningError(
            `GPG signing failed (code ${code}): ${error}`,
            new Error(error)
          ));
        }
      });
   ...
   */
  private async uploadToS3(archiveBuffer: Buffer, metadata: ArchiveMetadata): Promise<string> {
    try {
   ...
   */
  private generateS3Key(archiveId: string): string {
    const date = new Date();
  ...
  private async cleanupOldLogFiles(): Promise<void> {
    try {
      ...
      for (const file of files) {
        if (!file.endsWith('.log.old')) continue; // Only clean up rotated logs
   ...
   */
  private createEmptyArchiveMetadata(archiveId: string, _startDate: Date, _endDate: Date): ArchiveMetadata {
    return {
      archiveId,
      timestamp: new Date().toISOString(),
      logFileCount: 0,
  ...
  async listArchives(): Promise<ArchiveMetadata[]> {
    try {
      ...
      throw new ArchivalError(
        `Failed to list archives: ${error instanceof Error ? error.message : String(error)}`,
        'ARCHIVE_LIST_ERROR'
      );
    }
  }
 ...
 */
export function createLogArchiver(): LogArchiver {
 ...
 */
export async function runDailyArchival(): Promise<ArchiveResult> {
  try {
    ...
    throw new ArchivalError(
      `Daily archival failed: ${error instanceof Error ? error.message : String(error)}`,
      'DAILY_ARCHIVAL_ERROR'
    );
  }
}
 ...
 */
export async function archiveLogsInDateRange(
  startDate: Date,
  endDate?: Date
): Promise<ArchiveResult> {
  const archiver = createLogArchiver();
