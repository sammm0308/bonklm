 ...
 */
function computeEntryHash(timestamp: string, contentHash: string, previousHash: string): string {
 ...
 */
function loadChainState(logFile: string): ChainState {
  try {
    if (fs.existsSync(CHAIN_STATE_FILE)) {
      const content = fs.readFileSync(CHAIN_STATE_FILE, 'utf8');
      return JSON.parse(content) as ChainState;
    }
  } catch {
 ...
 */
function handleTamperingAlert(logFile: string, entryIndex: number, alertType: string): void {
...
export class HashChainManager {
  ...
  addEntry<T extends Record<string, unknown>>(logEntry: T): T & {
    ...
    try {
      ...
      // Compute hashes
      const timestamp = (logEntry['timestamp'] as string) || new Date().toISOString();
...
let chainManager: HashChainManager | null = null;

/**
 * Get or create the hash chain manager for the security log.
 */
export function getChainManager(logFile?: string): HashChainManager {
  if (logFile) {
 ...
 */
export function addChainFields<T extends Record<string, unknown>>(logEntry: T): T {
 ...
 */
export function verifySecurityLog(maxEntries?: number): VerificationResult {
 ...
 */
export function getIntegrityStatus(): Record<string, unknown> {
