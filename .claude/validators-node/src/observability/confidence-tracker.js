...
export class ConfidenceTracker {
    ...
    constructor() {
        ...
        for (const [severity, patterns] of Object.entries(UNCERTAINTY_MARKERS)) {
            this.compiledUncertainty.set(severity, patterns.map((p) => new RegExp(p, 'gi')));
        }
    ...
    calculateConfidenceScore(uncertaintyMarkers, confidenceBoosters, codeWarnings, attributions, textLength) {
        // Base score
        let score = 1.0;
    ...
    analyzeText(text) {
        ...
        // Calculate score
        const score = this.calculateConfidenceScore(uncertaintyMarkers, confidenceBoosters, codeWarnings, attributions, textLength);
        ...
        if (codeWarnings.length > 0) {
            analysisNotes.push(`Code contains ${codeWarnings.length} uncertainty indicators (TODO, FIXME, etc.)`);
        }
    ...
    recordAnalysis(result) {
        ...
        try {
            ...
            recordConfidenceAnalysis({
                ...
                confidenceLevel: result.confidenceLevel.toUpperCase(),
                hedgingPhrases: result.uncertaintyMarkers.map((m) => m.text).slice(0, 10),
                codeWarnings: result.codeWarnings.reduce((acc, w) => {
                    acc[w] = (acc[w] || 0) + 1;
                    return acc;
                }, {}),
                attributions: result.attributions.reduce((acc, a) => {
...
// ============================================================================
let confidenceTracker = null;
/**
 * Get or create the global confidence tracker instance.
 */
export function getConfidenceTracker() {
 ...
 */
export function analyzeResponseConfidence(text) {
    const tracker = getConfidenceTracker();
 ...
 */
export function getConfidenceIndicator(text) {
    if (!SHOW_CONFIDENCE) {
        return '';
    }
    const tracker = getConfidenceTracker();
...
export function analyzeToolOutput() {
    ...
    if (result.confidenceLevel === ConfidenceLevel.LOW ||
        result.confidenceLevel === ConfidenceLevel.VERY_LOW) {
        console.error(`[confidence_tracker] LOW_CONFIDENCE_RESPONSE: tool=${toolName}, level=${result.confidenceLevel}, score=${result.confidenceScore.toFixed(2)}`);
