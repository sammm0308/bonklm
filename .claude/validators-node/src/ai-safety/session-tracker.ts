...
/**
 * Multiple sessions persisted to disk.
 */
interface SessionsContainer {
  schema_version: number;
  sessions: Record<string, SessionPatternState>;
  last_cleanup: number;
}
 ...
 */
function loadSessionsContainer(): SessionsContainer {
  const sessionFile = getSessionFile();

  try {
    if (fs.existsSync(sessionFile)) {
      const content = fs.readFileSync(sessionFile, 'utf-8');
      const container = JSON.parse(content) as SessionsContainer;
    ...
    if (elapsed > 60000 && elapsed < SESSION_TIMEOUT_MS) {
      ...
      if (elapsed > DECAY_HALF_LIFE_MS / 2) { // Only decay categories after 5+ minutes
        ...
        for (const category of Object.keys(session.patterns_by_category)) {
          session.patterns_by_category[category] = Math.floor(
            (session.patterns_by_category[category] ?? 0) * categoryDecayFactor
          );
 ...
 */
function createFreshState(sessionId: string): SessionPatternState {
  return {
 ...
 */
function saveSessionsContainer(container: SessionsContainer): void {
  const sessionFile = getSessionFile();
 ...
 */
function saveSessionState(state: SessionPatternState): void {
  const container = loadSessionsContainer();
...
export function detectSlowDrip(
  ...
  state.finding_timestamps = state.finding_timestamps.filter(
