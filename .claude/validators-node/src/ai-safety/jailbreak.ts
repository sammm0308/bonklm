 ...
 */
export function detectHeuristicPatterns(text: string): HeuristicFinding[] {
 ...
 */
export function detectPatterns(content: string): JailbreakFinding[] {
 ...
 */
export function analyzeContent(content: string, sessionId?: string): JailbreakAnalysisResult {
  // 1. Normalize text
 ...
 */
export function validateJailbreak(
  content: string,
  toolName: string,
  sessionId?: string
): { exitCode: number; result: JailbreakAnalysisResult } {
  ...
  if (result.should_block) {
    ...
    AuditLogger.logBlocked(
      VALIDATOR_NAME,
      `Jailbreak attempt detected: ${result.findings[0]?.description || 'suspicious patterns'}`,
      content.slice(0, 200),
      {
 ...
 */
function formatFindings(result: JailbreakAnalysisResult): string {
  const lines: string[] = [];
...
export function main(): void {
  ...
  if (input.tool_name === 'UserPromptSubmit') {
    content = (input.tool_input as { prompt?: string }).prompt || '';
  } else {
    content = (input.tool_input.content as string) ||
              (input.tool_input.prompt as string) ||
              (input.tool_input.text as string) ||
