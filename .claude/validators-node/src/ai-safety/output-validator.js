...
import { AuditLogger, getProjectDir, } from '../common/index.js';
import { analyzeContent } from './prompt-injection.js';
import { analyzeWebContent, computeEffectiveSeverity, recordUrlFinding, } from './web-content-patterns.js';
 ...
 */
export function parsePostToolInput() {
    let raw;
    try {
 ...
 */
export function checkJsonDepth(obj, maxDepth = MAX_JSON_DEPTH, currentDepth = 0) {
 ...
 */
export function checkOutputSize(content) {
 ...
 */
export function getOutputToAnalyze(toolName, toolResponse) {
 ...
 */
function getContaminationFlagPath() {
 ...
 */
export function setContaminationFlag(toolName, reason) {
 ...
 */
export function isSessionContaminated() {
    try {
        const flagPath = getContaminationFlagPath();
        if (!fs.existsSync(flagPath))
            return false;
        const state = JSON.parse(fs.readFileSync(flagPath, 'utf-8'));
 ...
 */
export function validateOutput(toolName, toolResponse, toolInput) {
    ...
    if (toolName === 'WebFetch' && url && totalFindingCount > 0) {
        const highestSev = hasStandardBlock
    ...
    if (shouldBlock) {
        ...
        console.error(`[${VALIDATOR_NAME}] INJECTION DETECTED in ${toolName} output: ${findingSummary}`);
