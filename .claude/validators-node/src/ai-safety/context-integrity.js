 ...
 */
export function isContextFile(filePath) {
 ...
 */
export function readFileForScanning(filePath) {
    if (!filePath || typeof filePath !== 'string')
        return null;
 ...
 */
export function scanContextFile(filePath) {
...
export function main() {
    try {
        ...
        if (exitCode === EXIT_CODES.HARD_BLOCK) {
            AuditLogger.logBlocked(VALIDATOR_NAME, `CRITICAL injection detected in context file`, filePath, { finding_count: findingCount, severity });
            process.stderr.write(`[BMAD] BLOCKED: CRITICAL injection pattern detected in context file: ${filePath}\n`);
        }
        else if (exitCode === EXIT_CODES.SOFT_BLOCK) {
            process.stderr.write(`[BMAD] WARNING: Injection pattern detected in context file: ${filePath} (${findingCount} finding(s), severity: ${severity})\n`);
        }
