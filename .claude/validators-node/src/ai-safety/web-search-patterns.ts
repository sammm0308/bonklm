 ...
 */
const SEO_POISONED_PATTERNS: Array<{
  name: string;
  pattern: RegExp;
 ...
 */
const MALICIOUS_URL_PATTERNS: Array<{
  name: string;
  pattern: RegExp;
  severity: 'CRITICAL' | 'WARNING' | 'INFO';
  description: string;
 ...
 */
const TITLE_INJECTION_PATTERNS: Array<{
  name: string;
  pattern: RegExp;
  severity: 'CRITICAL' | 'WARNING' | 'INFO';
  description: string;
 ...
 */
export function analyzeSearchResults(content: string): SearchResultFinding[] {
  if (!content || content.trim().length === 0) {
    return [];
  }

  const findings: SearchResultFinding[] = [];
