...
// =============================================================================
// MAGIC NUMBER SIGNATURES (TPI-19)
// =============================================================================
...
interface MagicSignature {
  ...
  bytes: number[];
  offset: number;
  /** For WebP: additional bytes to check at offset 8 */
  extraBytes?: number[];
  extraOffset?: number;
}
...
const EXECUTABLE_SIGNATURES: MagicSignature[] = [
 ...
 * are detected.
 */
export function expandXmlEntities(content: string): string {
  ...
  for (const [name, value] of entities) {
    ...
    if (matches) {
      ...
      if (expansionCount > MAX_EXPANSIONS) break; // Prevent entity expansion bomb
 ...
 */
export function readFileHeader(filePath: string, maxBytes: number = MAX_HEADER_BYTES): Buffer | null {
  try {
...
export function detectMagicFormat(buffer: Buffer): string | null {
  for (const sig of MAGIC_SIGNATURES) {
    ...
    if (matches && sig.extraBytes && sig.extraOffset !== undefined) {
      if (buffer.length < sig.extraOffset + sig.extraBytes.length) continue;
...
export function validateMagicNumber(
  ...
  if (!IMAGE_EXTENSIONS.has(ext) || ext === '.svg') return findings;
 ...
 */
export function validateFileSize(filePath: string): MediaFinding[] {
  const findings: MediaFinding[] = [];
 ...
 */
export function extractJpegExifText(buffer: Buffer): Array<{ field: string; value: string }> {
 ...
 */
function extractIfdTextFields(buffer: Buffer, tiffStart: number): Array<{ field: string; value: string }> {
  const results: Array<{ field: string; value: string }> = [];

  if (tiffStart + 8 > buffer.length) return results;
  ...
  try {
    ...
    if (ifdStart + 2 > buffer.length) return results;
 ...
 */
export function extractPngTextChunks(buffer: Buffer): Array<{ field: string; value: string }> {
  const results: Array<{ field: string; value: string }> = [];
 ...
 */
export function extractXmpText(content: string): Array<{ field: string; value: string }> {
  const results: Array<{ field: string; value: string }> = [];
  const xmpPatterns: Array<{ field: string; regex: RegExp }> = [
 ...
 */
export function extractId3v2Text(buffer: Buffer): Array<{ field: string; value: string }> {
  const results: Array<{ field: string; value: string }> = [];

  // Check ID3v2 header
  if (buffer.length < 10) return results;
  if (buffer.toString('ascii', 0, 3) !== 'ID3') return results;
 ...
 */
export function extractWavInfoText(buffer: Buffer): Array<{ field: string; value: string }> {
  const results: Array<{ field: string; value: string }> = [];

  // Check RIFF/WAVE header
  if (buffer.length < 12) return results;
  if (buffer.toString('ascii', 0, 4) !== 'RIFF') return results;
  if (buffer.toString('ascii', 8, 12) !== 'WAVE') return results;

  const infoChunkIds: Record<string, string> = {
    'INAM': 'Title',
    'IART': 'Artist',
    'ICMT': 'Comment',
    'ICOP': 'Copyright',
 ...
 */
export function extractOggVorbisText(buffer: Buffer): Array<{ field: string; value: string }> {
  const results: Array<{ field: string; value: string }> = [];
  ...
  try {
    ...
    if (offset + 4 > buffer.length) return results;
 ...
 */
export function scanSvgContent(content: string): MediaFinding[] {
  const findings: MediaFinding[] = [];
 ...
 */
export function scanImageMetadata(
  filePath: string,
  buffer: Buffer,
): MediaFinding[] {
  const findings: MediaFinding[] = [];
  ...
  let textFields: Array<{ field: string; value: string }> = [];
  ...
  for (const { field, value } of textFields) {
    if (value === '[compressed text chunk]') continue; // Skip compressed
 ...
 */
export function scanAudioMetadata(
  filePath: string,
  buffer: Buffer,
): MediaFinding[] {
  const findings: MediaFinding[] = [];
...
interface MediaSessionState {
  ...
  image_loads: Array<{ path: string; timestamp: number; untrusted: boolean }>;
}

/**
 * Check if file is from an untrusted source.
 */
export function isUntrustedSource(filePath: string): boolean {
 ...
 */
function getMediaSessionStatePath(): string {
  const projectDir = getProjectDir();
 ...
 */
export function trackImageLoad(filePath: string, untrusted: boolean): MediaSessionState {
  const statePath = getMediaSessionStatePath();
  let state: MediaSessionState = { untrusted_image_count: 0, image_loads: [] };
 ...
 */
export function generateContextWarnings(
  filePath: string,
): MediaFinding[] {
  const findings: MediaFinding[] = [];
 ...
 */
export function computeHighestSeverity(findings: MediaFinding[]): Severity {
 ...
 */
export function scanMediaFile(
  filePath: string,
): { exitCode: number; findings: MediaFinding[]; severity: Severity } {
  // Not a media file — skip (no overhead on normal reads)
  if (!isMediaFile(filePath)) {
    return { exitCode: EXIT_CODES.ALLOW, findings: [], severity: 'INFO' };
  }

  const allFindings: MediaFinding[] = [];
  ...
  try {
    ...
    if (exitCode === EXIT_CODES.HARD_BLOCK) {
      AuditLogger.logBlocked(
        VALIDATOR_NAME,
        `CRITICAL media threat detected`,
        filePath,
        { finding_count: findings.length, severity },
      );
      process.stderr.write(
        `[BMAD] BLOCKED: CRITICAL threat detected in media file: ${filePath}\n`,
      );
    } else if (exitCode === EXIT_CODES.SOFT_BLOCK) {
      process.stderr.write(
        `[BMAD] WARNING: Suspicious content in media file: ${filePath} (${findings.length} finding(s), severity: ${severity})\n`,
      );
    }
