 ...
 */
export function isProtectedSettingsFile(filePath: string): [boolean, string | null] {
  ...
  const matchedProtected = PROTECTED_BASENAMES.find(
    (p) => p.toLowerCase() === basenameLower
  );
  ...
  const hasClaudeParent = segments.some(
    (seg) => seg === REQUIRED_PARENT
  );
 ...
 */
export function validateSettingsGuard(filePath: string): number {
...
export function main(): void {
  try {
    ...
    const toolInput = input.tool_input as unknown as WriteToolInput | EditToolInput;
