...
const CONTENT_SCANNING_PATTERNS: SecretPattern[] = [
 ...
 */
export function validateSecretGuard(content: string, filePath: string): number {
  if (!content) {
  ...
  // Block the operation
  const secretSummary = detections.slice(0, 3).map(d =>
    `- ${d.secretType} (${d.confidence}) at line ${d.lineNumber}`
  ).join('\n');
