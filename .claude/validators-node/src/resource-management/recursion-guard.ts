...
export class RecursionGuard {
   ...
   */
  private loadState(): RecursionState {
    try {
      if (fs.existsSync(this.stateFile)) {
        const content = fs.readFileSync(this.stateFile, 'utf-8');
        const state = JSON.parse(content) as RecursionState;
   ...
   */
  private initialState(): RecursionState {
    return {
      callStack: [],
      pathHistory: [],
      depthCounters: {},
   ...
   */
  private saveState(state: RecursionState): void {
   ...
   */
  private hashOperation(operation: string, target: string): string {
   ...
   */
  private detectCircularPattern(history: string[], newHash: string): [boolean, number] {
  ...
  checkDepth(type: string, depth: number, target: string = ''): RecursionCheckResult {
    const limitKey = type.replace(/_/g, '') as keyof RecursionLimits;
...
// Singleton instance
let recursionGuardInstance: RecursionGuard | null = null;

/**
 * Get or create the singleton recursion guard instance.
 */
export function getRecursionGuard(): RecursionGuard {
 ...
 */
export function checkRecursionLimit(type: string, depth: number, target: string = ''): [boolean, string] {
 ...
 */
export function checkCircularReference(operation: string, target: string): [boolean, string] {
  const guard = getRecursionGuard();
 ...
 */
function printBlockMessage(result: RecursionCheckResult): void {
  console.error('');
