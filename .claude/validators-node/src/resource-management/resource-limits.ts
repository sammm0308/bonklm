...
export class ResourceLimiter {
  ...
  private trackedProcesses: Map<number, TrackedProcess>;
  ...
  private loadState(): ResourceState {
    try {
      if (fs.existsSync(this.stateFile)) {
        ...
        const state = JSON.parse(content) as ResourceState;
   ...
   */
  private initialState(): ResourceState {
    return {
   ...
   */
  private saveState(state: ResourceState): void {
   ...
   * Check memory limits.
   */
  checkMemory(estimatedAdditionalMb: number = 0): ResourceCheckResult {
    ...
    let status: ResourceCheckResult['status'] = 'ok';
    let allowed = true;
    let message = '';
   ...
   */
  checkChildProcesses(startingNew: boolean = false): ResourceCheckResult {
   ...
   */
  checkFileSize(filePath: string, additionalBytes: number = 0): ResourceCheckResult {
   ...
   */
  trackProcess(pid: number, name: string = 'child'): void {
   ...
   */
  killProcess(pid: number, graceful: boolean = true): boolean {
...
// Singleton instance
let resourceLimiterInstance: ResourceLimiter | null = null;
 ...
 */
export function checkResourceLimits(): [boolean, string] {
 ...
 */
export function checkMemoryAvailable(estimatedMb: number): [boolean, string] {
  const limiter = getResourceLimiter();
 ...
 */
function printBlockMessage(result: ResourceCheckResult): void {
  console.error('');
