 ...
 */
export function truncateForRegex(input: string, maxLength: number = MAX_INPUT_LENGTH): {
  text: string;
  truncated: boolean;
  originalLength: number;
} {
...
export function safeTest(
  ...
  pattern: RegExp,
  validatorName?: string
): { result: boolean; truncated: boolean; executionTimeMs: number } {
 ...
 */
export function safeBatchMatch(
  input: string,
  patterns: RegExp[],
  validatorName?: string
): Array<{ pattern: RegExp; matches: RegExpMatchArray | null }> {
  ...
  const results: Array<{ pattern: RegExp; matches: RegExpMatchArray | null }> = [];
  let totalTimeMs = 0;
 ...
 */
export function getRegexTimeout(): number {
