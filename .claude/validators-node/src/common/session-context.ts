...
export interface SessionContextState {
  ...
  // Session-scoped permissions (inherited by subagents)
...
export interface PermissionCheckResult {
  allowed: boolean;
  reason: string;
  inherited: boolean;       // Was it inherited from parent session?
  sessionId: string;
  expiresIn?: number;       // Seconds until permission expires
}
...
export class SessionContext {
  ...
  static checkPermission(
    permissionType: string,
    options?: {
      consume?: boolean;        // Consume the permission if consumable
      validatorName?: string;   // For audit
    }
   ...
   */
  private static loadState(): SessionContextState | null {
    try {
      const contextFile = getContextFile();
      if (fs.existsSync(contextFile)) {
        const content = fs.readFileSync(contextFile, 'utf-8');
        return JSON.parse(content) as SessionContextState;
      }
    } catch {
   ...
   */
  private static saveState(state: SessionContextState): void {
    const contextFile = getContextFile();
 ...
 */
export function consumeSessionPermission(
  permissionType: string,
  validatorName?: string
): PermissionCheckResult {
