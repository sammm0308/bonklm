...
const VERIFY_MODE: VerifyMode = (process.env['BMAD_VERIFY_MODE'] as VerifyMode) || 'warn';
 ...
 */
function getCachedResult(filePath: string): CacheEntry | null {
 ...
 */
function setCachedResult(filePath: string, verified: boolean, reason: string): void {
...
export class SupplyChainVerifier {
  ...
  private manifestEntries: Map<string, ManifestEntry> = new Map();
  ...
  private _manifestSignatureValid: boolean | null = null;

  /** Cached signer ID from manifest verification */
  private _manifestSigner: string | null = null;
   ...
   */
  private verifyGpgSignature(): GpgResult {
...
/**
 * File categories for manifest organization.
 */
 ...
 */
export function addTrustedKey(keyPath: string): [boolean, string] {
 ...
 */
export function getVerifier(): SupplyChainVerifier {
 ...
 */
export function verifySkillIntegrity(skillId: string): [boolean, string] {
  const verifier = getVerifier();
 ...
 * @returns Tuple of [verified, message]
 */
export function verifyFileIntegrity(filePath: string): [boolean, string] {
  const verifier = getVerifier();
 ...
 */
export function validateSupplyChain(): number {
  ...
  if (!result.verified) {
    if (verifier.verifyMode === 'strict') {
      ...
      printWarning(
        `Supply Chain Warning: Verification failed for skill '${skillId}' - ${result.reason}. Proceeding anyway (BMAD_VERIFY_MODE=warn)`,
        skillId
      );
 ...
 * Handle CLI commands.
 */
function handleCliCommand(args: string[]): number {
