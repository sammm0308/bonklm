 ...
 */
export function getCachedClaims() {
    try {
 ...
 */
export function saveSessionClaims(claims) {
    try {
        const { SESSION_CLAIMS_FILE } = getPaths();
 ...
 */
export function parseClaimsFromOutput(output) {
    const claims = {};
 ...
 */
export function extractErrorFromOutput(stdout, stderr) {
    // Look for [FAIL] lines
 ...
 */
export function validateTokenWithScript() {
 ...
 * @returns Validation result
 */
export function validateRbac(claims, requiredRole) {
    if (!requiredRole) {
    ...
    if (userRoles.includes('security_lead') &&
 ...
 */
export function printAuthFailure(error) {
    printBlockMessage({
...
const isMain = process.argv[1]?.endsWith('token-validator.js') ||
